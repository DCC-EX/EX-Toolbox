...
public class FileReceiverActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (Intent.ACTION_SEND.equals(action) && type != null) {
            ...
            if (fileUri != null) {
                Log.d("EX_toolbox", "FileReceiverActivity: Received single file URI: " + fileUri);
                ...
                Log.e("EX_toolbox", "FileReceiverActivity: No URI found in SEND action");
            ...
            if (urisListParcelable != null && !urisListParcelable.isEmpty()) {
                ...
                if (!urisList.isEmpty()) {
                    Log.d("EX_toolbox", "FileReceiverActivity: Processing " + urisList.size() + " files.");
                    ...
                    Log.e("EX_toolbox", "FileReceiverActivity: No valid URIs found in SEND_MULTIPLE parcelable list");
                    updateUiForError("Error: No valid file data received for multiple files.");
                }
            } else {
                Log.e("EX_toolbox", "FileReceiverActivity: No URIs found in SEND_MULTIPLE action or list is empty");
                updateUiForError(getResources().getString(R.string.sharedFileNoUriReceived));
            }
        } else {
            Log.w("EX_toolbox", "FileReceiverActivity: Activity launched without a valid share intent.");
    ...
    private boolean doesFileExist(String fileName) {
        ...
        if (outputDir == null) {
            Log.w("EX_toolbox", "FileReceiverActivity: Cannot get external files dir to check existence.");
        ...
        executorService.execute(() -> {
            ...
            for (int i = 0; i < uris.size(); i++) {
                ...
                if (!resultMessage.startsWith("Error")) {
                    ...
                    errorCount++;
                    Log.e("EX_toolbox", "FileReceiverActivity: Error saving file " + currentFileNameForDisplay + ": " + resultMessage);
                }
            }
...
//        imageViewStatus.setImageResource(R.drawable.cross_glyph_dark);
    ...
    private String doSaveFileToScopedStorage(Uri uri, String filePrefix) {
        ...
        if (outputDir == null) {
            Log.e("EX_toolbox", "FileReceiverActivity: Failed to get app-specific external storage directory.");
            return "Error: Cannot access storage.";
        }
        if (!outputDir.exists() && !outputDir.mkdirs()) {
            Log.e("EX_toolbox", "FileReceiverActivity: Failed to create app-specific directory.");
        ...
        try (InputStream inputStream = contentResolver.openInputStream(uri);
            ...
            if (inputStream == null) {
                Log.e("EX_toolbox", "FileReceiverActivity: Failed to open input stream for URI: " + uri);
            ...
            Log.i("EX_toolbox", "FileReceiverActivity: File saved successfully: " + outputFile.getAbsolutePath());
            return "File saved: " + finalFileNameToSave; // Display the name it was actually saved as
        } catch (IOException e) {
            Log.e("EX_toolbox", "FileReceiverActivity: Error saving file: " + originalFileName, e);
            return "Error saving " + originalFileName + ": " + e.getMessage();
        } catch (SecurityException e) {
            Log.e("EX_toolbox", "FileReceiverActivity: Security exception, URI permission issue? " + uri, e);
    ...
    private String getFileName(Uri uri, ContentResolver contentResolver) {
        ...
        if (ContentResolver.SCHEME_CONTENT.equals(uri.getScheme())) {
            try (android.database.Cursor cursor = contentResolver.query(uri, null, null, null, null)) {
                ...
                Log.w("EX_toolbox", "FileReceiverActivity: Error getting filename from content URI", e);
            }
        }
