        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (Math.abs(deltaX) > MIN_DISTANCE) {
                    // left or right
                    if (deltaX < 0) {
                        Log.d("EX_Toolbox","SwipeDetector: Swipe Left to Right");
//                        Logger.show(Log.INFO,logTag, "Swipe Left to Right");
                        mSwipeDetected = Action.LR;
                        return true;
                    }
                    if (deltaX > 0) {
                        Log.d("EX_Toolbox","SwipeDetector: Swipe Right to Left");
//                        Logger.show(Log.INFO,logTag, "Swipe Right to Left");
                    ...
                    if (Math.abs(deltaY) > MIN_DISTANCE) {
                        // top or down
                        if (deltaY < 0) {
                            Log.d("EX_Toolbox","SwipeDetector: Swipe Top to Bottom");
//                            Logger.show(Log.INFO,logTag, "Swipe Top to Bottom");
                            mSwipeDetected = Action.TB;
                            return false;
                        }
                        if (deltaY > 0) {
                            Log.d("EX_Toolbox","SwipeDetector: Swipe Bottom to Top");
